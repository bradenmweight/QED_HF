...
def get_h1e_eri_f( f, WC, Emu, h1e, eri ):
    ...
    Z     = f**2 / 4 / WC
    G2    = np.subtract.outer( Emu, Emu )
    G4    = np.array([  p - q + r - s for p in Emu for q in Emu for r in Emu for s in Emu ]).reshape( (len(Emu),len(Emu),len(Emu),len(Emu)) )
    G2[:] = np.exp( -Z * G2 **2 )
    G4[:] = np.exp( -Z * G4**2 )
...
def __do_QED_VT_RHF_f( mol, LAM, WC, f=None, do_CS=True, return_wfn=False, initial_guess=None ):
    ...
    # # Rotate all relevant integrals back from dipole basis after VT Gaussian weighting
    h1e     = opt_einsum( "ap,pq,bq->ab", Umu, h1e, Umu )
    eri     = opt_einsum( "ap,bq,pqrs,cr,ds->abcd", Umu, Umu, eri, Umu, Umu )
